...
#Next line to be edited (use ('/Instructions.md') in the next line in web app)
...
file = st.file_uploader("Upload an image of food item", type=["jpg", "png"])
...
if file is None:
	...
	#test_image1 = np.asarray(test_image)
	#test_image2 = tf.convert_to_tensor(test_image1, dtype=tf.float32)
	test_image3 = np.expand_dims(test_image, 0)

	st.image(test_image, caption="Input Image", width = 400)

	#pred = predict_class(np.asarray(test_image), model)
	pred = predict_class(test_image3, model)
